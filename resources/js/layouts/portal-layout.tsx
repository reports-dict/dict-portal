...
export default function PortalLayout({ children }: PropsWithChildren) {
    ...
    // On narrow viewports the sidebar is a hidden-by-default overlay rather
    ...
    // Icon-only mode is a desktop-only concept now — on mobile the sidebar
    // is either fully hidden or fully open with real labels, never squeezed
    // to icons, since it no longer shares layout space with the content.
    ...
    return (
        <div className="flex h-screen flex-col overflow-hidden bg-neutral-950">
            {isNarrowViewport && !isFullscreen && (
                <header className="flex h-11 shrink-0 items-center gap-2.5 border-b border-neutral-800 bg-neutral-950 px-3 text-white">
                    <button
                        type="button"
                        onClick={() => setMobileMenuOpen(true)}
                        aria-label="Open menu"
                        className="flex h-8 w-8 items-center justify-center rounded-md text-neutral-300 transition-colors hover:bg-white/5 hover:text-white"
                    >
                        <Menu className="h-5 w-5" />
                    </button>
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded bg-white p-0.5">
                        <img
            ...
            <div className="flex min-h-0 flex-1 overflow-hidden">
                {!isFullscreen && (
                    <aside
                        className={
                            isNarrowViewport
                                ? `fixed inset-y-0 left-0 z-50 flex w-64 transform flex-col border-r border-neutral-800 bg-neutral-950 text-white transition-transform duration-200 ${
                                      mobileMenuOpen
                                          ? 'translate-x-0'
                                          : '-translate-x-full'
                                  }`
                                : `flex shrink-0 flex-col border-r border-neutral-800 bg-neutral-950 text-white transition-[width] duration-200 ${
                    ...
                    >
                        ...
                        <nav className="flex flex-1 flex-col gap-1 overflow-y-auto px-2 py-3">
                            <Link
                                href="/"
                                title={
                                    effectiveCollapsed ? 'Modules' : undefined
                                }
                        ...
                        <div className="shrink-0 border-t border-neutral-800 px-2 py-3">
                            ...
                            <Link
                                ...
                                as="button"
                                title={
                                    effectiveCollapsed ? 'Sign out' : undefined
                                }
                            ...
                            {!isNarrowViewport && (
                                <button
                                    ...
                                    title={
                                        collapsed ? 'Expand sidebar' : undefined
                                    }
                ...
                {isNarrowViewport && mobileMenuOpen && (
                    <div
