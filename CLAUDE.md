...
`dict-portal` is a Laravel 13 + Inertia.js v3 + React 19 portal that consolidates several previously-standalone operational tools for a container terminal (DICT) into one authenticated app: a vessel dashboard, a road queue display, a road queue ECD display, and a container yard viewer. All routes except `/login` require `auth` + `superadmin` middleware — there is currently no non-admin role, so every screen is gated behind `App\Models\User::isSuperadmin()`.

Authentication is LDAP-only against Active Directory (`directorytree/ldaprecord-laravel`), matched on `samaccountname`; there is no local password registration flow (see `App\Http\Controllers\Auth\LoginController` and `App\Models\User`, which implements `LdapAuthenticatable`).
...
`phpstan.neon` runs Larastan at level 7 with one deliberate suppression in `App\Http\Middleware\Superadmin`: LdapRecord-Laravel's auth config makes Larastan infer the wrong user model for the web guard, producing false "unreachable code" errors. If you touch that middleware and see new phpstan noise there, check whether it's already covered by the existing `ignoreErrors` pattern before adding a new suppression.
