...
class RoadQueueController extends Controller
{
    ...
    public function history(Request $request): Response
    {
        ...
        return Inertia::render('road-queue/history', [
            'tatHistory' => $tatHistory,
            'transactions' => $transactions,
            'filters' => [
                'tat_shift' => $this->queryString($request, 'tat_shift') ?? '',
                'tat_status' => $this->queryString($request, 'tat_status') ?? '',
    ...
    public function exportTatHistory(Request $request): StreamedResponse
    {
        ...
        return response()->streamDownload(function () use ($rows) {
            ...
            fputcsv($handle, [
                'Shift', 'Shift Start', 'Shift End', 'Type', 'Avg TAT', 'Seconds', 'Containers Processed', 'Recorded At',
            ]);
    ...
    public function exportHighElapsedTransactions(Request $request): StreamedResponse
    {
        ...
        return response()->streamDownload(function () use ($rows) {
            $handle = $this->openCsvOutput();
            fputcsv($handle, [
                'Container', 'Category', 'Precheck Time', 'Elapsed Time', 'CHE', 'ISO Type',
                'O/B Carrier', 'Freight Kind', 'Line Op', 'From', 'To', 'BAT#', 'First Captured', 'Last Seen',
            ]);
            ...
            foreach ($rows as $row) {
                fputcsv($handle, [
                    $row->container,
                    $row->category,
                    $this->formatManila($row->precheck_time),
                    $row->elapsed_time,
                    $row->assigned_che,
                    $row->type_iso,
                    $row->ob_carrier,
    ...
    {
        return DB::connection('mysql_operations_suite')
            ->table('road_queue_high_elapsed_transactions')
            ->when(
